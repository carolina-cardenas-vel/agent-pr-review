I want to create a custom agent using GitHub Copilot in VS Code. The agent should accept a Pull Request (PR) URL and the corresponding Azure DevOps (ADO) work item ID as inputs. It should automatically analyze all files changed in the PR, compare the implementation against the ADO work item requirements and acceptance criteria, and perform a comprehensive code review.

The review should determine:

- Whether the implementation fully satisfies the ADO work item requirements.
...
The implementation uses two existing MCP servers (GitHub and Azure DevOps) with a custom agent prompt in `.github/copilot-instructions.md` — no custom server code to maintain.